      ...
      <div className="border-b border-[#33ff33]/40 pb-2 mb-6 flex justify-between items-end px-2">
        <div>
          ...
          <div className="flex gap-4 mt-2">
            <button 
              onClick={() => setView('FX')}
              className={`text-xs uppercase font-bold tracking-wider px-2 py-1 border ${view === 'FX' ? 'bg-[#33ff33] text-black border-[#33ff33]' : 'text-[#33ff33] border-[#33ff33]/40'}`}
            >
              FX CORRELATIONS
            </button>
            <button 
            ...
            {view === 'FX' ? (
              initialData.slice(0, 5).map((rate, i) => (
                <div key={i} className="flex justify-between items-center text-sm border-b border-white/5 pb-2">
                  ...
                  <span className="text-[#33ff33] font-bold">{rate.exchange_rate}</span>
                </div>
              ))
            ) : (
              yieldChartData.slice(-1).map((point: any) => (
                <div key="latest-yields" className="space-y-2">
                  ...
                  <div className="flex justify-between items-center text-sm border-t border-[#33ff33]/20 pt-2 mt-2">
                    <span className="text-[#33ff33] font-bold">US 10Y</span>
                    <span className="text-[#33ff33] font-bold">{point['US']}%</span>
                  </div>
                </div>
              ))
            )}
